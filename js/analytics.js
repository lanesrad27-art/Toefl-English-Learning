...
export async function render(view) {
  ...
  try { pred = predictScores() } catch {}
  let sk = {}; try { sk = skillScores() } catch {}
  const progress = await DB.select('progress').catch(() => [])
  ...
  body.innerHTML =
    ...
    trend(progress) +
    card(`<h3>🔥 Consistency</h3><p>Current streak: <b>${getStreak()} day(s)</b> · Total sessions: <b>${progress.length}</b> · Errors logged: <b>${errors.length}</b></p>`)
}
